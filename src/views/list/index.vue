<template>
	<div id="book-list-container" :class="isPhoneClient ? 'mobileContainer' : 'pcContainer'">
		...
		<mu-dialog title="提示" max-width="80%" :esc-press-close="false"
		           :overlay-close="false" :open.sync="dialogStatus">
			此操作将删除该图书, 是否继续?
			<mu-button slot="actions" flat color="primary" @click="dialogStatus = false">取消</mu-button>
			<mu-button slot="actions" flat color="primary" @click="agree()">确定</mu-button>
		</mu-dialog>
	</div>
</template>
