  ...
  list: () => {
    const items = Mock.mock({
      'items|10': [
        {
          'id': '@id', // id
          'channel': '@cword(2, 8)', // 二级代理
          'contacts': '@cword(2, 4)', // 联系人姓名
          'phone': /^1[385][1-9]\d{8}/, // 手机号
          'totalAmount': '@integer(1, 100000)', // 总额
          'withdrawalAmount': '@integer(1, 100000)', // 已提现
          'unWithdrawalAmount': '@integer(1, 100000)', // 未提现
  ...
  detail: () => {
    const items = Mock.mock({
      'items': {
        ...
        'transferAccountRecord': '@image(\'120x160\', \'#4A7BF7\', \'icon\')',              //提现记录
        'status|1': [1, 2, 3],              //状态
